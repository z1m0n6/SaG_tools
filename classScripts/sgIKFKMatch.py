...
class SgIKFKMatch(object):
    ...
    @timeTaken
    def IKFKMatch(self, ik_CTRL, side, arm=None, foot=None):
        # do the matching
        if arm:
            orig = pm.getAttr(side + "_armSwitch_CTRL" + ".fkIkBlend")
            pm.setAttr(side + "_armSwitch_CTRL" + ".fkIkBlend", 0)
            ...
            oShoulder = pm.xform("{0}".format(side) + "_shoulder1_JNT", query=True, ws=True, matrix=True)
            oElbow = pm.xform("{0}".format(side) + "_elbow1_JNT", query=True, ws=True, matrix=True)
            oWrist = pm.xform("{0}".format(side) + "_hand1_JNT", query=True, ws=True, matrix=True)
            ...
            pos = pm.xform("{0}".format(side) + "_armFk_2_CTRL", query=True, ws=True, rp=True)
            pm.move("{0}".format(side) + "_armPv_CTRL", pos, rpr=True)
            ...
            pm.setAttr(side + "_armSwitch_CTRL" + ".fkIkBlend", orig)

        if foot:
            orig = pm.getAttr(side + "_legSwitch_CTRL" + ".fkIkBlend")
            pm.setAttr(side + "_legSwitch_CTRL" + ".fkIkBlend", 0)
            ...
            oHip = pm.xform("{0}".format(side) + "_hip1_JNT", query=True, ws=True, matrix=True)
            oKnee = pm.xform("{0}".format(side) + "_knee1_JNT", query=True, ws=True, matrix=True)
            oFoot = pm.xform("{0}".format(side) + "_foot1_JNT", query=True, ws=True, matrix=True)
            ...
            pm.setAttr(side + "_legSwitch_CTRL" + ".fkIkBlend", orig)
